             ...
             switch (true) {
                 ...
                 case a === '<':
                     ...
                     if ((this.nexttoken !== '') && (this.depth > 0)) {           //!depthend ) {
                        ...
                        llog = llog + this.nexttoken + '\n'
                        this.updatetoken(this.tokenobj.length-2, 'txtdata');                             // data update */
                     }
                     this.skiptoken();
                 ...
                 case a === '>':
                     ...
                     if (this.nexttoken === 'script'){
                         var idx = this.lines.indexOf('</script>');
                         this.txtdata = this.lines.substring(1, idx)
                         llog = llog + this.txtdata + '\n'
                         this.updatetokendata(this.tokenobj.length-1, 'txtdata');            // script data update
                         this.lines = this.lines.substring(idx + 9);
...
Parhtml.prototype.updatetokendata = function (index, key) {
         node = this.tokenobj[index];
         node[key] = this.txtdata;
         this.txtdata = '';
}
...
Parhtml.prototype.tostringtokens = function () {
	...
	for ( var i = 0; i < this.tokenobj.length; i++ ){
           n = this.tokenobj[i];
           if (n.txtdata == undefined) n.txtdata = '';
           str = str + n.depth + ' | ' + n.tagname + ' | ' + n.txtdata + ' | ' + n.tagdata + '\n';
	...
	for ( var i = 0; i < this.tokenobj.length; i++ ){
           ...
           if (node.tagname === 'script'){
              ...
              if (idx !== -1){
                  ...
                  //console.log("data = [%s]", node.txtdata);  // jsparser로 들어감
              }
           }
