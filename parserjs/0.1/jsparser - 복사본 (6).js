         ...
         this.getnexttoken = function () {
             ...
             switch (true) {
                 ...
                 case code === 59 || code === 0 || code === 10 || code === 13 || code === 0x2028 || code === 0x2029: // 0: eof, 59: ;
                     ...
                     /*
                       tokens에 넣기전에 추가적인 정보 (Type)를 넣는다.
                       Type 은 backupline 를 뒤진다. tokenarray 를 뒤져서 정보를 뽑는다.
                       사실 모든 타입정보는 여기서 넣을 수 있다.
                       이것은 inserttoken 함수 안에서 처리한다.
                     */
                 ...
                 case a === '[':                                                        // 배열에 해당
                     ...
                     if ((arr[0] || []).length > 0) {
                           // parsing 하지 않는다. data 자체는 의미가 없음
                           this.lines = this.lines.substring ((arr[0] || []).length);
                           this.tokenarray.unshift('AO');
                     }
...
jsparser.prototype.inserttoken = function() {
    ...
    if (this.tokenarray.length != 0) {
        ...
        if (this.tokenarray[1] === 'function') {              // general function ( function ab(){} )  [seq|function|ab]
            ...
            this.tokenarray.unshift('tp_vf');                 // var ab = function(){}
        } else if(this.findkeyfromarr('prototype') != -1) {
             this.tokenarray.unshift('tp_pf');                // prototype function
        } else if ((this.backupline.indexOf('.') > 0)                // object access : oa
                && (this.backupline.indexOf('(') > 0)
                && (this.findkeyfromarr('prototype') == -1)) {
            this.tokenarray.unshift('tp_oa');
        } else if (this.findkeyfromarr('new') > 0) {
            if (this.findkeyfromarr('array') > 0) {
                this.tokenarray.unshift('tp_na');             // new array
            } else {
                this.tokenarray.unshift('tp_no');             // new object
            }
        } else {
