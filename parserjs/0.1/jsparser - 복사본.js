...
var jsparser = function (hj) {
         ...
         this.lines = "ab = function() {   alert('ab')  {          {}        } }";
         ...
         this.getnexttoken = function () {
             ...
             switch (true) {
                 ...
                 case code === 0 || code === 10 || code === 13 || code === 0x2028 || code === 0x2029: // 0: eof
                     
                     this.tokenarray.unshift(++this.linenumber);
                     this.tokens.push(this.tokenarray);
                     console.log("code = [%s],  Line Number = [%d]", code, this.linenumber);
                     console.log("data = [%s]", this.tokenarray.toString());
                 ...
                 case a === '{':
                     if (this.keywords.function) {  // function block start   { { {  } } }
                        // 정규식 처리 ?
                        //var arr = this.lines.match(/^{[\{.\}]}$/ms);  //\\{(.*?)\\}
                        ...
                        //var arr = this.lines.match(/^.*?\{(.*)\}/s);
                        var arr = this.lines.match(/\{(.*)\}/s);
                        console.log("regexe====================> [%s]",arr.toString());
                     }
                     this.skiptoken(); 
                     break;
                 case a === '}':
