...
var jsparser = function (hj) {
         ...
         this.keywords = makeobject("var,let,const,function,new,isObj");
         ...
         this.getnexttoken = function () {
             ...
             switch (true) {
                 ...
                 case a === '{':
                     console.log("a=", a);
                     if (this.findkeyfromarr('function') > 1) {
                        console.log("varfunction", this.tokenarray[0]);
                        this.tokenarray.unshift('varfunction');
                     } else if (this.tokenarray[0] === 'function') {
                     //} else if (findkeyfromarr('function') > 1) {
                        console.log("genfunction", this.tokenarray[0]);
                        this.tokenarray.unshift('genfunction');
                     }
                     
                     var arr = this.lines.match(/\{(.*)\}/s);
                     if (this.keywords.function) {                                  
                     
                        this.lines = this.lines.substring (arr[0].length);   // 함수  { { {  } } } 부분을 모두 삭제
...
jsparser.prototype.findkeyfromarr = function(keywords) {
...
jsparser.prototype.inserttoken = function() {
                     ...
                     if (this.tokenarray.length != 0) {
                         ...
                         if (this.findkeyfromarr('new') > 0) {  // object type 를 배열에 추가.
                            this.tokenarray.unshift('newobj');
                         }
