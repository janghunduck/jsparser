         ...
         this.getnexttoken = function () {
             ...
             switch (true) {
                 ...
                 case a === '{':                                                   // 함수 or 객체에 해당 or let {ab, cd} = require('');
                     ...
                     let arr2 = this.currline.split("=");
                     let str = rangestr(arr2[0], '{', '}');
                     if (str !== ''){                                              // ok!  let {ab, cd} = require('');
                          let arr3 = str.split(',');
                          for(let i=0;i < arr3.length; i++){
                              this.tokenarray.push(arr3[i]);
                          }
                     } else {                                                       // ok! object, function
                         let arr = this.lines.match(/\{(.*)\}/s) || null || [];     // arr 에 0, 1 두개가 들어옴.
...
jsparser.prototype.getcurrline = function(){
    ...
    let idx1 = this.lines.indexOf('\n') + 1;
    let str = this.lines.substr(0, idx1);
    return str;
}
