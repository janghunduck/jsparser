         ...
         this.getnexttoken = function () {
             ...
             switch (true) {
                 ...
                 case a === ' ':
                     ...
                     console.log("nntoken==>[%s]", this.nntoken);
                     ...
                     if ((this.token != 'function') && (this.token != 'var')
                                                    && (this.token != this.prevtoken)
                                                    && (this.prevtoken != 'undefined')){
                          this.varrule(false);                                           // token이 keywords에 존재하는지 체크  keyword는 자료에 저장하지 않는다.
                 ...
                 case a === '(' || a === ')':                                              // || a === '[' || a === ']' || a === ',' || a === '.' || a === '^' || a === '*':
                     if (this.eatwhitespace()) break;
                     this.token = this.nexttoken;
                     console.log("(this.token)========>>[%s][%s]", this.token, a);
                     if ((this.token != 'function') && (this.token != 'var')
                                                    && (this.token != this.prevtoken)
                                                    && (this.prevtoken != 'undefined')){
                           this.keywords.function = true;
...
function substrnewline(input) {
   ...
   return input.substr(0, inewline);
}
...
function getnnttokenreg(input){
   ...
   if (!(result || []).length) { return result[0] } else {return [];}
}
...
Parjs.prototype.getnnttoken = function(input) {
       ...
       while (input) {
           ...
           if ((input.length === 0) || isNewLine(code) )  break;
           i++;
      }
