         ...
         this.getnexttoken = function () {
             ...
             switch (true) {
                 ...
                 case a === ' ':
                     ...
                     if (!this.checkkeywordinline('function')) { // 라인에서 function이 존재함.  ab = function 처리
                         if ((this.tokenseq == 0) && (this.token != 'function')) {                // 처음 토켄은 0 이다.
                             this.keywords.function = true;
                             this.functionrule(true);           
                             console.log("function add:", this.token);
                         }
                     }

                     if (!this.keywordexist(this.token)) {     // token이 keywords에 존재하는지 체크  keyword는 자료에 저장하지 않는다.
                          this.varrule(false);                 // (var x공백;)  (var x공백= 9;) 두 번째 공백이 들어올 경우 이것은 true 이다. 함수내부에서 keywords를 체크하기 때문에 체크를 따로 할 필요없음
                     }
                     if (this.token == 'var') { this.keywords.val = true; }        // token 이 keyword var 일때,
                 ...
                 case a === ',':                     
                     this.token = this.nexttoken;
                     this.varrule(true);                                           // var z, m;   // this.keywords.val 은 아직 true 여야한다.
                     this.skiptoken();
                     break;
                 ...
                 case a === '=':
                     ...
                     if (!this.keywordexist(this.token)) { if (this.keywords.val) { this.varrule2() } }                  // var   x=9;  = 이후 skipstrings
...
// 라인에서 'function' 같은 keyword가 있는지 체크
Parjs.prototype.checkkeywordinline = function(keystr){
