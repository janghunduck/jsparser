...
var Parjs = function (hj) {
         ...
         this.keywords = makeobject("var,let,const");
         ...
         this.getnexttoken = function () {
             ...
             switch (true) {
                 ...
                 case a === ' ':                                                   // 공백처리
                     ...
                     if (this.keywords.val) {         // (var x공백;)  (var x공백= 9;) 두 번째 공백이 들어올 경우 이것은 true 이다.
                         this.currobjseq < 0 ? this.newandupdate() : this.updatedata(this.tokenobj.length-1, 'vallist', this.token); // currobjseq < 0 : 생성된 오브젝트가 없다.
                         this.keywords.val = false;
                     }
                     
                     
                     if (this.token == 'var') {        // token 이 keyword var 일때,
                         this.keywords.val = true;
                     }
                     this.skiptoken();
                     break;
                 case a === ';':
                     ...
                     if (this.isglobal) {
                         if (this.keywords.val) {     // (var x;) 같이 중간에 공백이 없는 경우
                             this.currobjseq < 0 ? this.newandupdate() : this.updatedata(this.tokenobj.length-1, 'vallist', this.token);
                             this.keywords.val = false;
                         }
                     }
                 ...
                 case a === '=':
                     ...
                     if (this.isglobal) {
                         if (this.keywords.val) {                                     // (var x= 9;) 형식
                             this.currobjseq < 0 ? this.newandupdate() : this.updatedata(this.tokenobj.length-1, 'vallist', this.token);
                     ...
                     //this.skiptoken();
                     //this.skipstrings(';');  // = 뒤의 데이터를 다 지음,  변수함수에서는 aaa=function() 이어서 틀림
...
/**
Parjs.prototype.keywordvalchange = function (key) {
    //
    const keys = Object.keys(obj)   // 배열로 변경해서 값을 가져오고 값을 변경  다시 배열을 객체로 만들어....
...
Parjs.prototype.newandupdate = function() {
    this.newcollector();
    ...
    this.updatedata(this.tokenobj.length-1, 'vallist', this.token);
...
var pj = new Parjs("var   x=9; var y ;var z  ;");
