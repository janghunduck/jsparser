...
// var Parjs = this.Parjs = function(hj) { // 함수안에 this를 안써도 됨.
var Parjs = function (hj) {
         ...
         this.keywords = makeobject("var,let,const,function");
         ...
         this.getnexttoken = function () {
             ...
             switch (true) {
                 ...
                 //    break;
                 case a === ' ':
                     if (this.eatwhitespace()) break;   // 첫 공백이후 다시 공백이 들어오면 (연속된 공백은 지워준다.) var  x ;
                     this.token = this.nexttoken;       // 첫 공백일때
                     console.log("token==>[%s]", this.nexttoken);
                     ...
                     /**
                     if ((this.prevtoken = 'function') && (this.token != 'function')) {
                         if (!this.checkkeywordinline('function', this.token)) {                                  // 라인에서 function이 존재함.  ab = function 처리
                                 this.keywords.function = true;
                                 this.functionrule(true);
                                 console.log("function a add:", this.token);
                         } else {
                     ...
                     */
                     ...
                     console.log(":==============>[%s][%s]", this.prevtoken, this.nexttoken);
                     if ((this.token != 'function') && (this.token != 'var') && (this.token != this.prevtoken) && (this.prevtoken != 'undefined')){
                     ...
                     if (!this.checkkeywordinline('var', this.token)) {
                         if (!this.keywordexist(this.token)) {  this.varrule(false); }                                      // token이 keywords에 존재하는지 체크  keyword는 자료에 저장하지 않는다.
                     }                                                                  //    (var x공백;)  (var x공백= 9;) 두 번째 공백이 들어올 경우 이것은 true 이다. 함수내부에서 keywords를 체크하기 때문에 체크를 따로 할 필요없음
                 ...
                 case a === ',':                     
                     this.token = this.nexttoken;
                     this.varrule(true);                                                                                // var z, m;   // this.keywords.var 은 아직 true 여야한다.
                     this.skiptoken();
                     break;
                 ...
                 case a === '=':
                     ...
                     if (!this.keywordexist(this.token)) { if (this.keywords.var) { this.varrule2() } }                 // var   x=9;  = 이후 skipstrings 제거
                 ...
                 case a === '{':
                     // function ab(){  // '{' 에서 처리
                     if (this.keywords.function) this.skiptoken(); // 함수 내용 시작, 함수 안에서는 {} 를 따로 뽑아 필요한 정보만 뽑는다.
                     break;
                 case a === '#':
                 case a === '$':
                 case a === ':':
...
Parjs.prototype.functionrule = function(b){
    if (this.isglobal) {
        if (this.keywords.function) {
            this.currobjseq < 0 ? this.newandupdate('funlist') : this.updatedata(this.tokenobj.length-1, 'funlist', this.token); // currobjseq < 0 : 생성된 오브젝트가 없다.
            this.keywords.function = b;
            this.tokenseq++;
        }
    }
}
